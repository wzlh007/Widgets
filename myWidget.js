...
//当前相机信息获取
	...
	function doCompass()
	{
		...
		var centerY = (compasstop*1-offsetTop*1+compassheight/2*1)*1;
		//console.log(centerX);
		//console.log(centerY);
		//窗口变化，重算罗盘中点
		$(window).resize(function() {
			 offsetLeft = cesiumContainer.offsetLeft;
			 offsetTop = cesiumContainer.offsetTop;
			 cesiumwidth = document.getElementById("cesiumContainer").clientWidth;
			//console.log('cesiumwidth',cesiumwidth);
			 cesiumheight = document.getElementById("cesiumContainer").clientHeight;
			...
			//console.log('compasswidth',compasswidth);
			 ...
			 centerY = (compasstop*1-offsetTop*1+compassheight/2*1)*1;
		});
...
/*  		handler.setInputAction(function(movement) {
			...
			var mousedown = movement.position;
...
/* 		handler.setInputAction(function(movement) {
			...
			{	
				...
				console.log('startMousePosition move',startMousePosition);
				console.log('mousePosition move',mousePosition);
				...
				console.log('rad1',rad1*180/Math.PI);
				console.log('rad2',rad2*180/Math.PI);
 		...
 		$(document).ready(function(){
			$(document).mousemove(function(e){
				if(ring||looking){
					...
					//console.log('jquery mousePosition',mousePosition);
					if(ring){
						//console.log('startMousePosition move',startMousePosition);
						//console.log('mousePosition move',mousePosition);
						var rad1 = toRingRad(startMousePosition.x,startMousePosition.y,centerX,centerY);//Math.atan((startMousePosition.x-centerX)/(-startMousePosition.y+centerY));
						var rad2 = toRingRad(mousePosition.x,mousePosition.y,centerX,centerY);//Math.atan((mousePosition.x-centerX)/(-mousePosition.y+centerY));
						//console.log('rad1',rad1*180/Math.PI);
						//console.log('rad2',rad2*180/Math.PI);
						viewer.camera.setView({
							heading : (curheading-(rad2-rad1))
			...
			$(".compass-outer-ring").mousedown(function(e){
				ring = true;
				mousePosition=startMousePosition={x:e.pageX-offsetLeft,y:e.pageY-offsetTop};//transXY(offsetLeft,offsetTop,cesiumwidth,cesiumheight,e.clientX,e.clientY);
				//console.log('compass center',centerX,centerY);
				//console.log('startMousePosition click',startMousePosition);
				//console.log('mousePosition click',mousePosition);
